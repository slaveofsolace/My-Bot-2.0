...
| Home collection routes | Resource collectors, Loot Cart, a Treasury not-full check, and startup Daily Reward have separate bounded diagnostic routes with issued-input receipts and no-gems boundaries. The live-facing collectors route now permits at most one recognized mine/collector click per run. Collectors, Loot Cart, and both Daily Reward states now have verified recognition fixtures; Treasury still lacks one, and every account-affecting completion still needs its own supervised receipt. |
...
| **Events** | Clan Games, collectors, Loot Cart, Treasury, the startup Daily Reward, walls, and the supported lab policy |
