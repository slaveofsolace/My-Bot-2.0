...
| Treasury, achievements, challenge rewards, and free items | Legacy Home/Village maintenance functions | Not owned by a reviewed planner route; all are forced off during managed runs | One explicit closed-world route per reward surface, current-client fixtures, exact receipt, no-gems proof, Home restored |
