...
| Startup Daily Reward | `COCBot/functions/Run/OpenHomeCollectors.au3` | Claim and post-claim close recognition are fixture-verified. One supervised Claim committed with unchanged gems, but the first package did not recognize the red close state and required a manual reversible close; the source fix still needs rebuilt-package completion proof. | Rebuilt route issues one Claim, recognizes the exact red close, returns Home automatically, and preserves plan/profile/language and gem balance |
