...
Func CreateBotAndroid()
	...
	$y += 25
		...
		GUICtrlSetData(-1, GetTranslatedFileIni("Android", "CmbReplaceAdb", "Emulator|" & $g_sProductName))
		_GUICtrlSetTip(-1, GetTranslatedFileIni("Android", "CmbReplaceAdb_Info", "Configure if Android Emulator adb.exe should be replaced with " & $g_sProductName & " adb.exe to solve adb issues."))
	...
	GUICtrlCreateGroup(GetTranslatedFileIni("Android Control", "Group_04", "Emulator/Instance"), $x - 20, $y - 20, 255, 45)
		$g_hCmbAndroidEmulator = GUICtrlCreateCombo("", $x - 10, $y - 5, 82, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			_GUICtrlSetTip(-1, GetTranslatedFileIni("Android Control", "CmbAndroidEmulator_Info", "Change the emulator. " & $g_sProductName & " will automaticly detect the installed version."))
		...
		$g_hCmbAndroidInstance = GUICtrlCreateCombo("", $x + 77, $y - 5, 82, -1, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
			_GUICtrlSetTip(-1, GetTranslatedFileIni("Android Control", "CmbAndroidInstance_Info", "Change the instance. " & $g_sProductName & " will automaticly detect the available instance."))
