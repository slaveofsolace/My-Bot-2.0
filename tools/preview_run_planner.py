...
def check_design_constants(errors):
    ...
    expected = [
        ...
        (r'GUICtrlCreateButton\("Open center"', "native Open center button"),
        (r'GUICtrlCreateButton\("Load saved"', "native Load saved button"),
        (r'GUICtrlCreateButton\("Refresh"', "native Refresh button"),
        (r"\$y \+= 28", "status row below the native bridge buttons"),
    ]
...
def layout():
    ...
    boxes.append(Box("button", left, y, 70, 24, "Apply"))
    boxes.append(Box("button", left + 75, y, 55, 24, "Reset"))
    boxes.append(Box("button", left + 135, y, 105, 24, "Open center"))
    boxes.append(Box("button", left + 245, y, 100, 24, "Load saved"))
    boxes.append(Box("button", left + 350, y, 60, 24, "Refresh"))
    y += 28
    boxes.append(Box("status", left, y + 2, width, 18, "(bridge status)"))
