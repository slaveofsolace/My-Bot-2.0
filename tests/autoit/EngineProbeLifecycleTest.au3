...
AssertTrue(StringInStr($sParent, "Func _MBRFuncAutomaticProcessingPoolSize()", 1) = 0, "automatic processing pool does not synthesize an explicit thread count")
AssertTrue(StringInStr($sPool, "managed engine default processing pool (automatic)", 1) > 0, "automatic processing pool retains the managed default")
AssertTrue(StringInStr($sPool, "If $i < 1 Then", 1) > 0 And StringInStr($sPool, "Return True", 1) > StringInStr($sPool, "If $i < 1 Then", 1), "automatic processing pool returns before the managed export")
...
AssertTrue(StringInStr($sMax, "managed engine default parallelism (automatic)", 1) > 0, "automatic parallelism retains the managed default")
AssertTrue(StringInStr($sMax, "If $i < 1 Then", 1) > 0 And StringInStr($sMax, "Return True", 1) > StringInStr($sMax, "If $i < 1 Then", 1), "automatic parallelism returns before the managed export")
AssertTrue(StringInStr($sMax, "$i = -1", 1) = 0, "parallelism export never receives the blocking minus-one sentinel")
