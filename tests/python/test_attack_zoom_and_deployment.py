...
class AttackZoomAndDeploymentTests(unittest.TestCase):
    ...
    def test_zoom_is_immediate_bounded_and_reuses_the_upstream_adb_primitive(self):
        ...
        self.assertIn("For $iRedlineAttempt = 1 To 3", body)
        self.assertIn('SearchRedLines("ECD")', body)
        self.assertNotIn("SearchRedLines($CocDiamondECD)", body)
        self.assertIn("If $iRedlinePoints >= 50 Then ExitLoop", body)
        self.assertEqual(body.count("ForceCaptureRegion()"), 2)
        self.assertEqual(body.count("_CaptureRegions()"), 2)
        self.assertIn("If Not IsAttackPage(False) Then", body)
