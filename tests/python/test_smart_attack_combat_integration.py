...
class SmartAttackCombatIntegrationTests(unittest.TestCase):
    ...
    def test_hero_activation_is_armed_after_actor_proof_and_uses_fresh_coordinates(self):
        ...
        self.assertIn("no blind portrait click was sent", self.combat)
    ...
    def test_spell_casts_are_fresh_proven_and_bounded(self):
        ...
        self.assertRegex(self.combat, r"\$bAfterFound And \$iAfter >= \$iAmount")
        self.assertIn("Not $bAfterFound And $iAmount > 1", self.combat)
        self.assertIn("$bNeedSecondAbsenceProof", self.combat)
        self.assertIn("two independent valid absence scans", self.combat)
